...
def plot_jupiter_mapped():
    ...
    mapped_data = observation.get_mapped_data(
        degree_interval=_degree_interval
    )  # get the mapped data
    ...
    emission_map = observation.get_backplane_map(
        'EMISSION', degree_interval=_degree_interval
    )
    for idx in range(3):
